...
object LineOfSight {
	...
	def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
		...
		for (x <- from until until) {
			val angle = input(x) / x
			if (angle > currentMax) currentMax = angle
		}
		currentMax
	}
	...
	def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
		if (end - from > threshold) {
			val mid = (end + from) / 2
			val nodes = parallel(upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold))
			Node(nodes._1, nodes._2)
		} else {
